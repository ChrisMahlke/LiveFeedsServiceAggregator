...
def process_items(items):
    ...
    def process_item(item):
        ...
        if alfp_data is None:
            ...
            status_code = StatusManager.get_status_code("000", statusCodesDataModel)
            ...
            if all([agol_is_valid, item_is_valid, service_is_valid, layers_are_valid]):
                ...
                if last_update_timestamp_diff_minutes > avg_update_int_threshold:
                    status_code = StatusManager.get_status_code("001", statusCodesDataModel)
                ...
                if last_run_timestamp_diff_minutes > avg_feed_int_threshold:
                    status_code = StatusManager.get_status_code("002", statusCodesDataModel)

                # 003 Check
                if alfp_data["lastStatus"]["code"] == 2:
                    if alfp_data["consecutiveFailures"] > int(value["consecutive_failures_threshold"]):
                        status_code = StatusManager.get_status_code("003", statusCodesDataModel)

                # 004 Check
                if alfp_data["lastStatus"]["code"] == 3:
                    if alfp_data["consecutiveFailures"] > int(value["consecutive_failures_threshold"]):
                        status_code = StatusManager.get_status_code("004", statusCodesDataModel)

                # 005 Check
                if alfp_data["lastStatus"]["code"] == 1:
                    if alfp_data["consecutiveFailures"] > int(value["consecutive_failures_threshold"]):
                        status_code = StatusManager.get_status_code("005", statusCodesDataModel)

                # 006 Check
                if alfp_data["lastStatus"]["code"] == -1:
                    status_code = StatusManager.get_status_code("006", statusCodesDataModel)

                # 100
                # Check retry count
                if retry_count > int(value["default_retry_count"]):
                    status_code = StatusManager.get_status_code("100", statusCodesDataModel)
                ...
                if elapsed_time > avg_elapsed_time_threshold:
                    status_code = StatusManager.get_status_code("101", statusCodesDataModel)
                ...
                if any([agol_is_valid, item_is_valid, service_is_valid]):
                    if service_is_valid:
                        ...
                        if item_is_valid:
                            ...
                            # 102
                            status_code = StatusManager.get_status_code("102", statusCodesDataModel)
                        # 201 Check
                        if layers_are_valid is not True:
                            status_code = StatusManager.get_status_code("201", statusCodesDataModel)
                    else:
                        # 500
                        if item_is_valid:
                            status_code = StatusManager.get_status_code("500", statusCodesDataModel)
                        else:
                            print(f"Item | Fail")
                            # If ALL of the Service states are False, we have reached a critical failure in the system
                            status_code = StatusManager.get_status_code("501", statusCodesDataModel)
                else:
                    # If ALL of the Service states are False, we have reached a critical failure in the system
                    status_code = StatusManager.get_status_code("501", statusCodesDataModel)
        ...
        value["status"]["code"] = status_code

        return current_item[0], {**current_item[1]}

    return dict(map(process_item, items))
...
if __name__ == "__main__":
    ...
    if fileExist:
        ...
        for config_key, config_value in data_model_dict.items():
            print(f"{config_key}")
            ...
            for ele in FileManager.open_file(outputFilePath)["items"]:
                ...
                if config_key == status_file_key:
                    merged_dict = {**ele, **config_value}
                    data_model_dict.update({
                        config_key: merged_dict
                    })
    ...
    print("===================================================================")
    process_items(data_model_dict.items())
